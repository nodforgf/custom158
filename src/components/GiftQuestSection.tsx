...
export default function GiftQuestSection({ onBack, onFinish }: { onBack: () => void, onFinish: () => void }) {
    ...
    const GIFTS = [
        "ยอมตามใจทั้งวันเลย 🌹", "เลี้ยงบิงซูรสชาติที่เค้าชอบ 555 🍨", "อยู่ด้วยกัน 2 คน 😍", "ไปเที่ยวด้วยกันไหม ? 🌊",
        "ให้ของขวัญชิ้นนึง 🤍", "ยอมให้เทอเลี้ยงหนัง 1 เรื่อง 555 🎬", "จะไม่งอน 1 วัน 😘", "อยากขออะไรจาก จินนี่ก็พูดมาเลย นายจ๋า~ 🎨"
    ];
              ...
              {result && (
                ...
                >
                  <motion.div 
                    ...
                    className="bg-[#fff8fb] p-8 md:p-12 rounded-[40px] text-center w-full max-w-sm shadow-[0_20px_80px_rgba(232,120,154,0.3)] border-8 border-[#f4a7be] flex flex-col items-center justify-center min-h-[380px]"
                  >
                    <motion.span 
                      animate={{ y: [0, -15, 0] }}
                    ...
                    <p className="text-[#e8789a]/60 text-[10px] font-bold uppercase tracking-[0.2em] mb-3">Congratulations!</p>
                    
                    {/* 🎯 แก้ไขตรงนี้: ใช้ Dynamic Class เช็กความยาวตัวอักษรเพื่อลดขนาดฟอนต์คูปองใบที่ยาวเป็นพิเศษ */}
                    <h3 className={`text-[#c2547a] font-black uppercase italic mb-6 break-words px-2 text-center ${
                      result.length > 50 
                        ? 'text-[15px] leading-relaxed tracking-normal normal-case' 
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-[#e8789a] text-white font-black rounded-2xl hover:bg-[#c2547a] transition-colors uppercase tracking-widest text-sm mt-auto"
                    >
                      Keep it Forever
                    </button>
                  </motion.div>
